...
<template>
  <NSpace vertical :size="16">
    <!-- 页头 -->
    <NCard :bordered="false" class="card-wrapper border-primary/20 bg-cn-bg">
      <div class="flex items-center justify-between flex-wrap gap-12px">
        <div>
          ...
          <p class="text-cn-text-secondary text-14px m-0">{{ $t('page.themeShowcase.subtitle') }}</p>
        </div>
        <NSpace>
          <NTag class="!bg-cn-green-100/12 !border !border-cn-green-100/35 !text-cn-green-aux" :bordered="false">
            Cornex 楚能
          </NTag>
    ...
    <NTabs v-model:value="activeTab" type="line" animated>
      ...
      <NTabPane name="colors" :tab="$t('page.themeShowcase.tabs.colors')">
        <NSpace vertical :size="16">
          <NCard :bordered="false" class="card-wrapper">
            <div class="text-cn-md font-600 mb-16px">{{ $t('page.themeShowcase.semanticColors') }}</div>
            <NGrid cols="2 s:3 m:5" :x-gap="12" :y-gap="12" responsive="screen">
              <NGi v-for="item in semanticColors" :key="item.name">
                <div class="flex flex-col items-center gap-6px">
                  ...
                  />
                  <span class="text-cn-sm font-mono text-cn-text-secondary">{{ item.label }}</span>
          ...
          <NCard :bordered="false" class="card-wrapper">
            <div class="text-cn-md font-600 mb-16px">{{ $t('page.themeShowcase.blueScale') }}</div>
          ...
          <NCard :bordered="false" class="card-wrapper">
            <div class="text-cn-md font-600 mb-16px">{{ $t('page.themeShowcase.greenScale') }}</div>
            <NGrid cols="5 s:10" :x-gap="8" :y-gap="8" responsive="screen">
          ...
          <NCard :bordered="false" class="card-wrapper">
            <div class="text-cn-md font-600 mb-16px">{{ $t('page.themeShowcase.textColors') }}</div>
          ...
          <NCard :bordered="false" class="card-wrapper">
            <div class="text-cn-md font-600 mb-16px">{{ $t('page.themeShowcase.typography') }}</div>
            <NSpace vertical :size="12">
              <div class="text-cn-xl text-cn-text-primary">特大标题 24px / 36px</div>
              <div class="text-cn-lg text-cn-text-primary">文章标题 20px / 30px</div>
              <div class="text-cn-md text-cn-text-primary">小标题 / 按钮 / 表头 16px / 24px</div>
              <div class="text-cn-base text-cn-text-primary">正文 / 表单 / 列表 14px / 20px</div>
              <div class="text-cn-sm text-cn-text-hint">次要信息 / 辅助说明 12px / 18px</div>
            </NSpace>
          </NCard>

          <NCard :bordered="false" class="card-wrapper">
            <div class="text-cn-md font-600 mb-16px">{{ $t('page.themeShowcase.auxColors') }}</div>
            <NGrid cols="2 s:3 m:5" :x-gap="12" :y-gap="12" responsive="screen">
          ...
          <NCard :bordered="false" class="card-wrapper">
            <div class="text-cn-md font-600 mb-16px">{{ $t('page.themeShowcase.chartColors') }}</div>
          ...
          <NCard :bordered="false" class="card-wrapper">
            <div class="text-cn-md font-600 mb-16px">{{ $t('page.themeShowcase.controls.radius') }}</div>
            <p class="text-cn-sm text-cn-text-hint mb-16px">{{ $t('page.themeShowcase.controls.radiusHint') }}</p>
            <div class="flex flex-wrap gap-24px mb-16px">
              <div v-for="item in radiusSpec" :key="item.px" class="flex flex-col items-center gap-8px">
                ...
                <div class="text-center">
                  <div class="text-cn-base text-cn-text-primary font-600">{{ item.px }}px</div>
                  <div class="text-cn-sm text-cn-text-hint">{{ item.label }}</div>
            ...
            <div class="grid grid-cols-1 m:grid-cols-3 gap-12px">
              <div
                v-for="item in radiusSpec"
                :key="item.usage"
                class="rd-cn-md bg-cn-bg border border-cn-border p-16px"
              >
                <div class="text-cn-sm mb-4px">{{ item.label }}</div>
                <div class="text-cn-text-secondary text-14px">{{ item.usage }}</div>
              </div>
            </div>
          </NCard>

          <NCard :bordered="false" class="card-wrapper">
            <div class="text-cn-md font-600 mb-16px">{{ $t('page.themeShowcase.controls.buttonHeight') }}</div>
            <p class="text-cn-sm text-cn-text-hint mb-16px">{{ $t('page.themeShowcase.controls.buttonHeightHint') }}</p>
            <div class="flex flex-wrap items-end gap-24px mb-16px">
              <div v-for="item in buttonHeightSpec" :key="item.size" class="flex flex-col items-center gap-8px">
                ...
                <div class="text-center">
                  <div class="text-cn-sm font-mono">{{ item.size }} · {{ item.px }}px</div>
                  <div class="text-cn-sm text-cn-text-hint max-w-160px">{{ item.desc }}</div>
                </div>
              </div>
            </div>
          </NCard>
          ...
          <NCard :bordered="false" class="card-wrapper">
            <div class="text-cn-md font-600 mb-16px">{{ $t('page.themeShowcase.generalColors') }}</div>
            <NGrid cols="2 s:3 m:5" :x-gap="12" :y-gap="12" responsive="screen">
      ...
      <NTabPane name="buttons" :tab="$t('page.themeShowcase.tabs.buttons')">
        <NCard :bordered="false" class="card-wrapper">
          <div class="text-cn-md font-600 mb-12px">{{ $t('page.themeShowcase.buttonVariants') }}</div>
          <p class="text-cn-sm text-cn-text-hint mb-16px">{{ $t('page.themeShowcase.buttonHint') }}</p>

          <NSpace vertical :size="20">
            <div>
              <div class="text-cn-text-secondary text-14px mb-8px">
                设计稿 · 主按钮交互色（type="primary" 自动应用）
              </div>
              <div class="flex flex-wrap gap-16px mb-12px">
                <div v-for="item in buttonInteractionColors" :key="item.label" class="flex items-center gap-8px">
                  ...
                  <div>
                    <div class="text-cn-sm">{{ item.label }}</div>
            ...
            <div>
              ...
              <div class="flex flex-wrap items-center gap-12px">
                ...
                <NButton tertiary>+ 新增任务</NButton>
                <NButton type="primary">+ 新增</NButton>
              </div>
              <div class="mt-8px" :class="[codeHintClass]">type="primary" / type="default" / tertiary</div>
            </div>

            <div>
              <div class="text-cn-text-secondary text-14px mb-8px">状态：禁用 / 成功</div>
              <div class="flex flex-wrap items-center gap-12px">
                <NButton type="primary" disabled>禁用</NButton>
                <NButton type="success">成功</NButton>
                <NButton type="default">取消·成功</NButton>
                <NButton tertiary>+ 新增任务·成功</NButton>
            ...
            <div>
              <div class="text-cn-text-secondary text-14px mb-8px">其他风格</div>
              <div class="flex flex-wrap items-center gap-12px">
                <NButton type="success">能源强调</NButton>
                <NButton class="!bg-cn-bg !border !border-cn-border !border-dashed !text-cn-text-secondary font-mono">
                  INDUSTRIAL-01
                </NButton>
                <NButton class="!text-primary hover:!bg-cn-hover" text>文字按钮</NButton>
            ...
            <div>
              ...
              <div class="flex flex-wrap items-end gap-16px">
                <div v-for="item in buttonHeightSpec" :key="item.size" class="flex flex-col items-center gap-6px">
                  <NButton type="primary" :size="item.size">
      ...
      <NTabPane name="tags" :tab="$t('page.themeShowcase.tabs.tags')">
        <NCard :bordered="false" class="card-wrapper">
          <div class="text-cn-md font-600 mb-12px">{{ $t('page.themeShowcase.deviceStatusTags') }}</div>
          <p class="text-cn-sm text-cn-text-hint mb-16px">
      ...
      <NTabPane name="feedback" :tab="$t('page.themeShowcase.tabs.feedback')">
        <NSpace vertical :size="16">
          <NCard :bordered="false" class="card-wrapper">
            <div class="text-cn-md font-600 mb-16px">{{ $t('page.themeShowcase.alerts') }}</div>
            <NSpace vertical :size="12">
              <NAlert type="info" title="系统通知">产线 A 今日计划产量 12,000 电芯，当前完成 68%。</NAlert>
              <NAlert type="success" title="运行正常">BMS-001 电池包 SOC 87%，温度 32.5℃，状态良好。</NAlert>
              <NAlert type="warning" title="温度预警">BMS-003 电芯温度 48.6℃，接近告警阈值 50℃。</NAlert>
              <NAlert type="error" title="严重告警">PCS-002 过流保护触发，请立即检查！</NAlert>
              <NAlert class="!border-l-4 !border-l-primary !bg-cn-bg font-mono" type="info" title="[SYS] 工业日志">
          ...
          <NGrid cols="1 m:2" :x-gap="16" :y-gap="16" responsive="screen">
            <NGi>
              <NCard :bordered="false" class="card-wrapper">
                <div class="text-cn-md font-600 mb-16px">{{ $t('page.themeShowcase.inputs') }}</div>
                <p class="text-cn-sm text-cn-text-hint mb-16px">{{ $t('page.themeShowcase.formHint') }}</p>
                <NSpace vertical :size="16">
                  <NFormItem label="公司名称" label-placement="top">
                    <NInput v-model:value="formCompany" placeholder="请输入公司名称" />
                  </NFormItem>
                  <NFormItem label="产线选择" label-placement="top">
                    <NSelect v-model:value="selectValue" :options="selectOptions" placeholder="选择产线" />
                  </NFormItem>
                  <NFormItem label="输入中（聚焦态）" label-placement="top">
                    <NInput v-model:value="inputValue" placeholder="聚焦时显示主题蓝边框" />
                  </NFormItem>
                  <NFormItem label="校验失败" label-placement="top" validation-status="error" feedback="请输入有效内容">
                    <NInput v-model:value="errorInputValue" status="error" placeholder="错误态红色边框" />
                  </NFormItem>
                </NSpace>
              </NCard>
            </NGi>
            <NGi>
              <NCard :bordered="false" class="card-wrapper">
                <div class="text-cn-md font-600 mb-16px">{{ $t('page.themeShowcase.progress') }}</div>
                <NSpace vertical :size="16">
                  <div>
                    <div class="text-cn-sm mb-4px">SOC 87%</div>
                    <NProgress type="line" :percentage="87" indicator-placement="inside" processing />
                  </div>
                  <div>
                    <div class="text-cn-sm mb-8px">{{ $t('page.themeShowcase.pagination') }}</div>
                    <NPagination v-model:page="paginationPage" :page-count="8" />
                  </div>
                  <div>
                    <div class="text-cn-sm mb-8px">{{ $t('page.themeShowcase.checkboxSwitch') }}</div>
                    <NSpace vertical :size="12">
                  ...
                  <NSpace>
                    <NBadge :value="3">
                      <NButton type="default" size="small">告警消息</NButton>
                    </NBadge>
                  </NSpace>
                </NSpace>
              </NCard>
      ...
      <NTabPane name="layout" :tab="$t('page.themeShowcase.tabs.layout')">
        <NSpace vertical :size="16">
          <NGrid cols="1 s:3" :x-gap="16" :y-gap="16" responsive="screen">
            <NGi>
              <NCard title="标准卡片" class="card-wrapper" :bordered="false">
                <p class="text-cn-text-secondary text-14px m-0">class="card-wrapper"</p>
              </NCard>
            </NGi>
            <NGi>
              <NCard title="高亮卡片" class="card-wrapper border-primary/20 bg-cn-bg" :bordered="false">
                <p class="text-cn-text-secondary text-14px m-0">border-primary/20 bg-cn-bg</p>
              </NCard>
            </NGi>
            <NGi>
              <NCard
                ...
                :bordered="false"
              >
                <div class="text-cn-xl text-cn-text-primary text-primary font-mono">12,580</div>
                <div class="text-cn-sm text-cn-text-hint">今日产量 (PCS)</div>
              </NCard>
            </NGi>
          </NGrid>

          <NCard :bordered="false" class="card-wrapper">
            <div class="text-cn-md font-600 mb-16px">{{ $t('page.themeShowcase.modal') }}</div>
            <div class="flex flex-wrap items-center gap-12px">
              <NButton type="primary" @click="showModal = true">打开 Modal</NButton>
              <NButton type="default" @click="showDialog = true">打开 Dialog</NButton>
            </div>
          </NCard>

          <NCard :bordered="false" class="card-wrapper">
            <div class="text-cn-md font-600 mb-16px">{{ $t('page.themeShowcase.table') }}</div>
            <NDataTable :columns="columns" :data="tableData" :bordered="false" size="small" />
            <div class="mt-12px" :class="[codeHintClass]">NDataTable 由 Naive 主题令牌统一样式</div>
          </NCard>
        </NSpace>
      </NTabPane>
      ...
      <NTabPane name="industrial" :tab="$t('page.themeShowcase.tabs.industrial')">
        <NSpace vertical :size="16">
          <NCard :bordered="false" class="card-wrapper">
            <div class="text-cn-md font-600 mb-12px">{{ $t('page.themeShowcase.industrial.charts') }}</div>
            <p class="text-cn-sm text-cn-text-hint mb-16px">{{ $t('page.themeShowcase.industrial.chartsHint') }}</p>
            <IndustrialCharts />
          </NCard>

          <NCard :bordered="false" class="card-wrapper">
            <div class="text-cn-md font-600 mb-12px">{{ $t('page.themeShowcase.industrial.scrollbar') }}</div>
            <p class="text-cn-sm text-cn-text-hint mb-12px">{{ $t('page.themeShowcase.industrial.scrollbarHint') }}</p>
            <div class="rd-cn-md bg-cn-bg border border-cn-border cn-scrollbar h-240px">
              <div
                v-for="item in scrollbarDemoItems"
                :key="item.id"
                class="flex items-center justify-between py-10px px-8px border-b border-cn-border last:border-b-0 hover:bg-cn-hover rd-4px text-cn-base text-cn-text-primary"
              >
                <span class="font-mono">{{ item.name }}</span>
                <span class="text-cn-sm text-cn-text-hint">SOC {{ item.soc }}</span>
                <NTag
                  size="small"
                  :bordered="false"
    ...
    <!-- Modal -->
    <NModal v-model:show="showModal" preset="card" title="设备详情" style="width: 480px">
      <NSpace vertical :size="12">
        <NAlert type="info">BMS-001 电池管理系统 · 运行正常</NAlert>
